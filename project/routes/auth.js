...
router.post('/login', async function(req, res) {
  try {
    ...
    if(ans && ans[0] ){
      ...
      const user_last_time = await images_utils.getLastTime(ans[0]);
      const finish_rate = await images_utils.getRate(ans[0]);
      res.status(200).send({Id:req.session.user_id, IsAdmin: ans[1], FullName: ans[2], globalSettings: globalSettings, user_score:user_score, last_time: user_last_time, numRanked: finish_rate, is_submitted:ans[3],is_done:ans[4],ranked:ans[5],unranked:ans[6],extras:ans[7]});
    }
    else
